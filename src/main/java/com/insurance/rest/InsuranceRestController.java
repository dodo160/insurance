...
public class InsuranceRestController {
	...
	public ResponseEntity<InsuranceDTO> getInsuranceById(@PathVariable("id") final Long id) {
		return new ResponseEntity<>(insuranceMapper.insuranceToInsuranceDTO(insuranceService.findById(id)), HttpStatus.OK);
	}

	@GetMapping("/insurances")
	public ResponseEntity<Set<InsuranceDTO>> getAllInsurances() {
		return new ResponseEntity<>(insuranceService.findAll().stream().map(x->insuranceMapper.insuranceToInsuranceDTO(x)).collect(Collectors.toSet()), HttpStatus.OK);
	}
	...
	public ResponseEntity<Void> addInsuracnce(@RequestBody final InsuranceDTO insuranceDTO) throws ValidationException {
		insuranceService.add(insuranceMapper.insuranceDTOtoInsurance(insuranceDTO));
	...
	public ResponseEntity<InsuranceDTO> updateInsurance(@RequestBody final InsuranceDTO insuranceDTO) throws ValidationException {
		return new ResponseEntity<>(insuranceMapper.insuranceToInsuranceDTO(insuranceService.update(insuranceMapper.insuranceDTOtoInsurance(insuranceDTO))), HttpStatus.OK);
	}
	...
	public ResponseEntity<BigDecimal> calculateInsurance(@RequestBody final InsuranceDTO insuranceDTO) throws ValidationException {
		return new ResponseEntity<>(insuranceService.calculateInsurance(insuranceMapper.insuranceDTOtoInsurance(insuranceDTO)), HttpStatus.OK);
	}
}
